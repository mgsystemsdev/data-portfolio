...
def init_db():
    ...
    try:
        ...
        pass
    try:
        conn.execute("ALTER TABLE system_state ADD COLUMN handout_mode INTEGER NOT NULL DEFAULT 0")
    except sqlite3.OperationalError:
        pass
    try:
        conn.execute("ALTER TABLE system_state ADD COLUMN active_handout TEXT")
    except sqlite3.OperationalError:
        pass
    try:
        conn.execute("ALTER TABLE system_state ADD COLUMN handout_phase TEXT NOT NULL DEFAULT 'drill'")
    except sqlite3.OperationalError:
        pass
    try:
        conn.execute("ALTER TABLE system_state ADD COLUMN handout_challenges_done INTEGER NOT NULL DEFAULT 0")
    except sqlite3.OperationalError:
        pass
...
def all_handout_concepts_locked(state):
    """Check if all concepts in the active handout are locked for the current topic/category."""
    handout_raw = state.get("active_handout")
    if not handout_raw:
        return False
    try:
        data = json.loads(handout_raw) if isinstance(handout_raw, str) else handout_raw
    ...
    existing = get_topic_concepts(topic, category)
    locked_names = {c["concept_name"] for c in existing if c.get("locked")}
    return all(name in locked_names for name in concepts)

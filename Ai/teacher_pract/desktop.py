"""
Concept Practice Engine — Flet Desktop App
...
def main(page: ft.Page):
    page.title = "Concept Practice Engine"
    ...
    challenge_topic_dropdown = ft.Dropdown(
        options=[ft.dropdown.Option(key="all", text="(All topics)")] +
                [ft.dropdown.Option(key=t, text=t) for t in TOPIC_MENU.keys()],
        value="all",
        width=200,
        text_size=12,
    )

    challenge_tier_dropdown = ft.Dropdown(
        options=[
            ft.dropdown.Option(key="Guided", text="🟢 Guided"),
            ft.dropdown.Option(key="Semi-Guided", text="🟡 Semi-Guided"),
            ft.dropdown.Option(key="Independent", text="🔴 Independent"),
        ],
        value="Guided",
        width=200,
        text_size=12,
    ...
    def start_challenge(e):
        nonlocal session_id, messages
        scope = challenge_topic_dropdown.value
        tier = challenge_tier_dropdown.value
        challenge_msg = (
            f"── CHALLENGE MODE ──────────────────────\n"
            f"🔥 Mode: Challenge\n"
            f"📚 Scope: {scope}\n"
        ...
        messages.clear()
        chat_list.controls.clear()
        save_message(session_id, "assistant", challenge_msg, topic=scope, concept=None, status="complete")
        messages.append({"role": "assistant", "content": challenge_msg})
        chat_list.controls.append(make_bubble("assistant", challenge_msg))
        refresh_state()
        page.update()
    ...
    state_panel = ft.Container(
        content=ft.Column(
            [
                ...
                ft.Container(height=12),
                ft.Text("🔥 Challenge Mode", size=12, weight=ft.FontWeight.BOLD, color=ft.Colors.RED_300),
                challenge_topic_dropdown,
                challenge_tier_dropdown,
                ft.Button("⚡ Start Challenge", on_click=start_challenge, width=200),
    ...
    title_bar = ft.Container(
        content=ft.Row(
            [
                ft.Text("🎯 Concept Practice Engine", size=18, weight=ft.FontWeight.BOLD),
                ft.Text("One drill at a time.", size=12, color=ft.Colors.WHITE38, italic=True),
            ],
